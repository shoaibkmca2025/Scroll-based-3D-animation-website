...
/* The society model and its scroll-driven camera, ported from society3d.js.
   Same geometry, same lighting, same camera stops and easing — only the
   plumbing changed: it is a plain function with a teardown instead of a
   custom element, and the six procedural noise canvases the original built
   but never assigned to a material are gone. */
...
export function createSociety(host) {
  ...
  // rendered once instead of every frame.
  ...
  const cyl = (rt, rb, h, mat, x, y, z, seg, name) => {
  ...
  function tower(w, d, floors, x, z, name) {
    ...
    const inst = new THREE.InstancedMesh(winGeo, M.glass, floors * perRow * 2);
    ...
    for (let f = 1; f < floors; f += 1) {
      g.add(box(w * 0.42, 0.12, 1.1, M.slab, w * 0.22, f * fh + 0.1, d / 2 + 0.5, name + '-balcony'));
  ...
  world.add(ring);
  world.add(box(5, 0.04, 16, M.road, 0, 0.02, 26, 'entry-road'));
  ...
  world.add(tower(10.5, 10.5, 11, 0, -20, 'wing-c'));
  ...
  poolG.add(box(11.8, 0.24, 7.3, M.slab, 0, 0.1, 0, 'pool-coping'));
  ...
  // street lamps and AC units — instanced
  {
    const dummy = new THREE.Object3D();
    const lampPos = [];
    for (let i = 0; i < 10; i++) {
